...
func listenVoiceCommandHandler(s *discordws.WellensittichSession, i *discordgo.InteractionCreate) {
	...
	listeningNow := vc.VoiceReceiver.ToggleListen(vc)

	if listeningNow {
		s.EventGuildFeatureFunction("listenCommandOn", g.ID, i.ChannelID)
		err = ic.DefaulInteractionAnswer("Listening now!")
		if err != nil {
			fmt.Printf("Error responding to interaction: %v\n", err)
		}
	} else {
		s.EventGuildFeatureFunction("listenCommandOff", g.ID, i.ChannelID)
		err = ic.DefaulInteractionAnswer("Stopped listening!")
		if err != nil {
			fmt.Printf("Error responding to interaction: %v\n", err)
		}
	}
